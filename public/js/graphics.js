(() => {
  ...
  const opcionesSubcategoria = filtros.subcategoria
    ? Array.from(filtros.subcategoria.options).slice(1).map((opcion) => ({
      value: opcion.value,
      text: opcion.text,
  ...
  const opcionSubcategoriaPlaceholder = filtros.subcategoria?.options[0]?.cloneNode(true)
  ...
  const chartBalanceContainer = document.getElementById('graphics-balance-donut');
  const chartEvolutionContainer = document.getElementById('graphics-evolution');
  const chartGoalsEvolutionContainer = document.getElementById('graphics-goals-evolution');
  const chartCategoriesContainer = document.getElementById('graphics-categories');
  const chartIncomeCategoriesContainer = document.getElementById('graphics-income-categories');

  if (!chartBalanceContainer || !chartEvolutionContainer || !chartGoalsEvolutionContainer || !chartCategoriesContainer || !chartIncomeCategoriesContainer) {
    return;
  }

  const cardIncomeCategories = document.getElementById('graphics-income-card');
  const cardExpensesCategories = document.getElementById('graphics-expenses-card');
  ...
  const chartIncomeCategories = new ApexCharts(chartIncomeCategoriesContainer, {
    chart: {
      type: 'bar',
      height: 360,
      toolbar: { show: false },
  ...
  Object.values(filtros)
    .filter((elemento) => elemento && elemento !== filtros.limpiar && elemento !== filtros.categoria)
    .forEach((elemento) => {
  ...
  function pintarSubcategorias() {
    ...
    const categoriaSeleccionada = filtros.categoria ? filtros.categoria.value : '';
    const valorActual = filtros.subcategoria.value;

    filtros.subcategoria.innerHTML = '';
    filtros.subcategoria.appendChild(opcionSubcategoriaPlaceholder.cloneNode(true));
    ...
    const sigueDisponible = opcionesSubcategoria.some(
      (opcion) => opcion.value === valorActual && opcion.categoriaId === categoriaSeleccionada,
    );
  ...
  function actualizarPanel() {
    ...
    const maxEvolucion = Math.max(...evolucion.ingresos, ...evolucion.gastos.map(v => Math.abs(v)), 300);
    ...
    chartEvolution.updateSeries([
      ...
      { name: 'Gastos', data: evolucion.gastos.map((valor) => Math.abs(valor)) },
    ]);

    const evolucionObjetivos = agruparEvolucionObjetivos(transaccionesFiltradas);
    ...
    const topIngresos = agruparIngresoPorCategoria(transaccionesFiltradas, 8, categoriaSeleccionada);
    ...
    const topCategorias = agruparGastoPorCategoria(transaccionesFiltradas, 8, categoriaSeleccionada);
    ...
    if (cardIncomeCategories) {
      cardIncomeCategories.classList.toggle('is-disabled', topIngresos.importes.length === 0);
    }

    if (cardExpensesCategories) {
      cardExpensesCategories.classList.toggle('is-disabled', topCategorias.importes.length === 0);
    }

    if (cardGoalsEvolution) {
      cardGoalsEvolution.classList.toggle('is-disabled', evolucionObjetivos.aportes.length === 0 && evolucionObjetivos.retiros.length === 0);
    }
  }
  ...
  function filtrarTransacciones(transacciones) {
    ...
    const idCategoria = filtros.categoria ? Number.parseInt(filtros.categoria.value, 10) : 0;
    const idSubcategoria = filtros.subcategoria ? Number.parseInt(filtros.subcategoria.value, 10) : 0;
    const idMetodo = filtros.metodo ? Number.parseInt(filtros.metodo.value, 10) : 0;
    ...
    return transacciones.filter((tx) => {
      ...
      if (idCategoria > 0 && Number.parseInt(tx.id_categoria, 10) !== idCategoria) {
        return false;
      }

      if (idSubcategoria > 0 && Number.parseInt(tx.id_subcategoria, 10) !== idSubcategoria) {
        return false;
      }
  ...
  function calcularTotales(transacciones) {
    return transacciones.reduce((acumulado, tx) => {
  ...
  function pintarKpis(totalesBase) {
    ...
    if (kpis.balance) {
      ...
      kpis.balance.classList.remove('is-negative', 'is-warning', 'is-success');
  ...
  function agruparPorMes(transacciones) {
    ...
    const llavesOrdenadas = Array.from(mapa.keys()).sort((a, b) => a.localeCompare(b));

    return {
      etiquetas: llavesOrdenadas,
      ingresos: llavesOrdenadas.map((llave) => (mapa.get(llave) || {}).ingresos || 0),
      gastos: llavesOrdenadas.map((llave) => (mapa.get(llave) || {}).gastos || 0),
    };
  }
  ...
  function agruparEvolucionObjetivos(transacciones) {
    ...
    return {
      etiquetas: llavesOrdenadas,
      aportes: llavesOrdenadas.map((llave) => (mapa.get(llave) || {}).aporte || 0),
      retiros: llavesOrdenadas.map((llave) => (mapa.get(llave) || {}).retiro || 0),
    };
  }

  function agruparGastoPorCategoria(transacciones, limite, categoriaSeleccionada) {
    const mapa = new Map();
    ...
    transacciones.forEach((tx) => {
      ...
      if (categoriaSeleccionada) {
        ...
        if (String(tx.id_categoria) === categoriaSeleccionada) {
          llave = (tx.nombre_subcategoria || 'Sin subcategoría').trim() || 'Sin subcategoría';
        } else {
          return;
        }
      } else {
        llave = (tx.nombre_categoria || 'Sin categoría').trim() || 'Sin categoría';
      }
  ...
  function agruparIngresoPorCategoria(transacciones, limite, categoriaSeleccionada) {
    const mapa = new Map();
